...
class	NN
{
	...
	public:
		...
		void					feedForwardMultiClass(float (*f)(const float&), float (*derivatedF)(const float&));
		...
		float					backPropagationMultiClass(float (*f)(const float&), float (*derivatedF)(const float&), const std::vector<std::vector<float>>& targets);
		...
		void					trainMultiClass(const size_t& epochs, const std::vector<std::vector<float>>& inputs, const std::vector<std::vector<std::vector<float>>>& expectedOutputs, float (*f)(const float&), float (*derivatedF)(const float&));
		std::vector<float>			use(const std::vector<float>& inputs, float (*activHL)(const float&), float (*activO)(const float&));
		std::vector<std::vector<float>>		useMultiClass(const std::vector<float>& inputs, float (*f)(const float&), float (*derivatedF)(const float&));
